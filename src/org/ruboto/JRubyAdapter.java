...
public class JRubyAdapter {
	...
	private static String localContextScope = "SINGLETON"; // FIXME(uwe): Why
															// not CONCURRENT ?
															// Help needed!
	...
	public static String getScriptFilename() {
		return (String) callScriptingContainerMethod(String.class,
				"getScriptFilename");
	}
	...
	public static Object runRubyMethod(Object receiver, String methodName,
			Object... args) {
		try {
			Method m = ruby.getClass().getMethod("runRubyMethod", Class.class,
	...
	public static <T> T runRubyMethod(Class<T> returnType, Object receiver,
	...
	public static void put(String name, Object object) {
		try {
			Method putMethod = ruby.getClass().getMethod("put", String.class,
					Object.class);
	...
	public static Object runScriptlet(String code) {
		try {
			Method runScriptletMethod = ruby.getClass().getMethod(
	...
	public static synchronized boolean setUpJRuby(Context appContext,
		...
		if (!initialized) {
			...
			Log.d("Setting up JRuby runtime ("
					+ (isDebugBuild ? "DEBUG" : "RELEASE") + ")");
			System.setProperty("jruby.backtrace.style", "normal"); // normal raw
																	// full mri
			System.setProperty("jruby.bytecode.version", "1.6");
			// System.setProperty("jruby.compat.version", "RUBY2_0"); // RUBY1_9
			// is the default in JRuby 1.7
			// System.setProperty("jruby.compile.backend", "DALVIK");
			System.setProperty("jruby.compile.mode", "OFF"); // OFF OFFIR JITIR?
																// FORCE FORCEIR
			...
			System.setProperty("jruby.ji.upper.case.package.name.allowed",
					"true");
			System.setProperty("jruby.class.cache.path",
					appContext.getDir("dex", 0).getAbsolutePath());
			...
			try {
				...
				if (RUBOTO_CORE_VERSION_NAME != null
						&& (RUBOTO_CORE_VERSION_NAME.equals("0.4.7") || RUBOTO_CORE_VERSION_NAME
								.equals("0.4.8"))) {
				...
				// This needs to be set before the ScriptingContainer is
				// initialized
				...
				Class rubyClass = Class.forName("org.jruby.Ruby", true,
						scriptingContainerClass.getClassLoader());
				Class rubyInstanceConfigClass = Class.forName(
						"org.jruby.RubyInstanceConfig", true,
						scriptingContainerClass.getClassLoader());

				Object config = rubyInstanceConfigClass.getConstructor()
						.newInstance();
				rubyInstanceConfigClass.getMethod("setDisableGems",
						boolean.class).invoke(config, true);
				rubyInstanceConfigClass.getMethod("setLoader",
						ClassLoader.class).invoke(config, classLoader);

				if (output != null) {
					rubyInstanceConfigClass.getMethod("setOutput",
							PrintStream.class).invoke(config, output);
					rubyInstanceConfigClass.getMethod("setError",
							PrintStream.class).invoke(config, output);
				}

				// This will become the global runtime and be used by our
				// ScriptingContainer
				rubyClass.getMethod("newInstance", rubyInstanceConfigClass)
						.invoke(null, config);
				...
				Class scopeClass = Class.forName(
						"org.jruby.embed.LocalContextScope", true,
						scriptingContainerClass.getClassLoader());
				Class behaviorClass = Class.forName(
						"org.jruby.embed.LocalVariableBehavior", true,
						scriptingContainerClass.getClassLoader());

				ruby = scriptingContainerClass.getConstructor(scopeClass,
						behaviorClass).newInstance(
						Enum.valueOf(scopeClass, localContextScope),
				...
				// callScriptingContainerMethod(Void.class, "setClassLoader",
				// classLoader);
				Method setClassLoaderMethod = ruby.getClass().getMethod(
				...
				if (appContext.getFilesDir() != null) {
					String defaultCurrentDir = appContext.getFilesDir()
							.getPath();
					Log.d("Setting JRuby current directory to "
							+ defaultCurrentDir);
					callScriptingContainerMethod(Void.class,
	...
	public static Boolean addLoadPath(String scriptsDir) {
		if (new File(scriptsDir).exists()) {
			...
			callScriptingContainerMethod(Void.class, "setCurrentDirectory",
					scriptsDir);
			return true;
		} else {
	...
	@SuppressWarnings("unchecked")
	private static <T> T callScriptingContainerMethod(Class<T> returnType,
			String methodName, Object... args) {
	...
	public static boolean isRubyOneNine() {
		...
		return rv.startsWith("2.1.") || rv.startsWith("2.0.")
				|| rv.startsWith("1.9.");
	}
	...
	private static String scriptsDirName(Context context) {
		...
		if (isDebugBuild()) {
			...
			if (storageDir == null
					|| (!storageDir.exists() && !storageDir.mkdirs())) {
